...
public class ECElGamalEncryptorTest extends UsagePatternTestingFramework {
	...
	@Test
	public void testECElGamalEncryptorOne() {
		ECPoint data = priKey.getParameters().getG().multiply(n);
		ECEncryptor encryptor = (ECEncryptor) new ECElGamalEncryptorTest();
        encryptor.init(pRandom);
        encryptor.encrypt(data);
        
        Assertions.mustBeInAcceptingState(encryptor);
	}
	
	@Test
	public void testECElGamalEncryptorTwo() {
		ECPoint data = priKey.getParameters().getG().multiply(n);
		ECEncryptor encryptor = (ECEncryptor) new ECElGamalEncryptorTest();
        encryptor.init(pRandom);
        encryptor.encrypt(data);
	...
	//Error
	public void testECElGamalEncryptorThree() {
		ECPoint data = priKey.getParameters().getG().multiply(n);
		ECEncryptor encryptor = (ECEncryptor) new ECElGamalEncryptorTest();
        encryptor.init(pRandom);
        encryptor.encrypt(data);
        ...
        Assertions.mustNotBeInAcceptingState(encryptor);
	}
}
